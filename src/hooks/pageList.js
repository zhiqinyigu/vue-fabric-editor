...
const repoSrc = process.env.VUE_APP_APIHOST;
...
function getMaterialInfoUrl(info) {
  const imgUrl = (info && info.data && info.data.attributes && info.data.attributes.url) || '';
...
function getMaterialPreviewUrl(info) {
  ...
  const url = info && info.data && info.data.attributes && info.data.attributes.url;
  const imgUrl = (formats && formats.small && formats.small.url) || url || '';
  return repoSrc + imgUrl;
}
    ...
    try {
      ...
      const list = res.data.data.map((item) => {
        return {
          ...
          json: item.attributes ? item.attributes.json : undefined,

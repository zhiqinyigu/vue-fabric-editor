<template>
  <Modal v-model="visible" title="图片裁剪" width="80%" @on-ok="onOk" @on-cancel="onCancel">
    <div class="main">
      <Spin v-if="loading" size="large" fix>图片加载中...</Spin>
      <div class="options">
        <label>裁剪比例</label>
...
export default {
  ...
  setup() {
    ...
    // vue-cropper 图片加载完成/失败（state: success | error）
    function onImgLoad(state) {
      loading.value = false;
      if (state === 'error') {
    ...
    function changeRatio(ratio) {
      ...
      nextTick(() => {
        cropperRef.value && cropperRef.value.goAutoCrop(9999, 9999);
      });
    }

    ...
    <div class="my-tabs__header p-0.5 mb-3 rounded bg-gray-100 cursor-pointers">
      <div class="my-tabs__header-shell relative flex justify-between">
        <div
          ...
          :class="{ 'my-active': tab.label === value }"
...
<script>
import { ref, reactive, onMounted, watch, nextTick, provide } from '@vue/composition-api';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function changeTab(index = -1) {
      if (index < 0) {
        index = tabs.value.findIndex((tab) => tab.label === props.value);
      }
      ...
      try {
        await nextTick();
        preActiveTabVM && preActiveTabVM.changeActive(false);
        preActiveTabVM = tabs.value[index];
        preActiveTabVM && preActiveTabVM.changeActive(true);
      } catch (error) {
        console.log(error);
      }
    }

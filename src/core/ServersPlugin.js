...
class ServersPlugin {
  ...
  _applyAssetManifest(json) {
    ...
    const replace = (items) => {
      ...
      items.forEach((item) => {
        if (!item || typeof item !== 'object') return;
        if (item.type === 'image' && typeof item.src === 'string' && !isVariableSrc(item.src)) {
  ...
  _toDataURLSafe(option) {
    try {
      ...
      const isTainted =
        e &&
        (e.name === 'SecurityError' || /tainted|insecure|cross-origin|CORS/i.test(e.message || ''));
      if (isTainted) {
        const err = new Error(
  ...
  clear() {
    ...
    workspacePlugin &&
      workspacePlugin._clearBackgroundImageState &&
      workspacePlugin._clearBackgroundImageState();

...
function changeTextWidth(eventData, transform, x, y) {
  ...
  var localPoint = controlsUtils.getLocalPoint(
    transform,
    transform.originX,
    transform.originY,
    x,
...
function changeTextHeight(eventData, transform, x, y) {
  ...
  var anchor =
    transform.originY === 'bottom'
      ? target.translateToOriginPoint(target.getCenterPoint(), transform.originX, transform.originY)
      : null;
...
function changeTextSize(eventData, transform, x, y) {
  ...
  if (target.clipEnabled) {
    ...
    var anchor = target.translateToOriginPoint(
  ...
  if (target.autoGrow) {
    ...
    var oldMaxH = Number(target.autoGrowMaxHeight) || 0;
    var oxGrow = transform.originX === 'right' ? 'right' : 'left';
    var anchorGrow = target.translateToOriginPoint(target.getCenterPoint(), oxGrow, 'top');
    target.set('width', newW);
    target.set({ autoGrowMaxHeight: newH > 0 ? newH : null });
    target.set('dirty', true);
    target.initDimensions && target.initDimensions();
    target.setPositionByOrigin(anchorGrow, oxGrow, 'top');
    return oldW !== target.width || oldMaxH !== (Number(target.autoGrowMaxHeight) || 0);
  }
  // 无尺寸锁无自增高：顶部恒固定（Y 不漂移），仅改宽度；旋转感知
  var oxTop = transform.originX === 'right' ? 'right' : 'left';
  var anchorTop = target.translateToOriginPoint(target.getCenterPoint(), oxTop, 'top');
  target.set('width', newW);
  target.setPositionByOrigin(anchorTop, oxTop, 'top');
  return true;
}
...
function makeTextControls() {
  ...
  controls.ml = new fabric.Control({
    x: -0.5,
    y: 0,
    cursorStyleHandler: scaleSkewCursor,
    actionHandler: cw,
    actionName: 'resizing',
  });
  ...
  controls.tl = new fabric.Control({
    x: -0.5,
    y: -0.5,
    cursorStyleHandler: scaleCursor,
    actionHandler: cs,
    actionName: 'resizing',
  });
...
fabric.Textbox = fabric.util.createClass(fabric.Textbox, {
  ...
  _render: function (ctx) {
    var needsClip =
      this.clipEnabled || (this.autoGrow && (this.autoGrowMinHeight || this.autoGrowMaxHeight));
  ...
  _wrapLine: function (_line, lineIndex, desiredWidth, reservedSpace) {
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      if (wordWidth > desiredWidth && word.length > 1) {
        ...
        var chunk = [],
          chunkW = 0,
          chunkOffset = offset - word.length,
          segMax = 0;
  ...
  _ellipsisCapable: function () {
    return (
      ...
      ((this.clipEnabled && this.frameHeight) || (this.autoGrow && this.autoGrowMaxHeight))
    );
  },

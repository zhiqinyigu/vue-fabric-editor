...
// 裁切 shell（SimpleClipImagePlugin 的临时交互层）不参与吸附对齐：
// shell 与宿主图片边缘几乎恒重合（4px 容差内必命中边缘线），命中时本插件会
// 程序化改写 shell 的 scaleX/scaleY/位置，而 clipPath 同步走 shell 自身事件
// （先于画布 object:* 事件），吸附修改在后 → 裁切框漂移。故对裁切 shell 全程跳过。
const isClipShell = (obj) => get(obj, 'clip') === true;
...
class AlignGuidLinePlugin {
  ...
  init() {
    ...
    canvas.on('mouse:down', (e) => {
      // 裁切 shell 不捕获对齐基线（吸附后续处理一并跳过）
      if (e && (e.target === undefined || isClipShell(e.target))) return;
      ...
      try {
        ...
        // 取目标对象属性失败时忽略，对齐线按上一帧值计算
      }
    });
    canvas.on('object:moving', (e) => {
      // 裁切 shell 跳过吸附（其拖动位置由 shell 自身事件同步到 clipPath，不得被程序化修改）
      if (viewportTransform === undefined || e.target === undefined || isClipShell(e.target))
        return;
    ...
    canvas.on('object:scaling', (e) => {
      // 裁切 shell 跳过吸附：镜像拖角场景下吸附会改写/移动 shell，裁切框即刻漂移
      if (viewportTransform === undefined || e.target === undefined || isClipShell(e.target))
        return;
      const activeObject = e.target;
      const canvasObjects = canvas.getObjects();
...
AlignGuidLinePlugin.pluginName = 'AlignGuidLinePlugin';

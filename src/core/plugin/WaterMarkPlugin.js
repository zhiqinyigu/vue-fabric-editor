...
class WaterMarkPlugin {
    constructor(canvas, editor) {
        ...
        this.drawing = {
            ...
            [POSITION.full]: (width, height, cb) => {
                ...
                let patternCanvas = this.createCanvas(this.drawOps.isRotate ? textW * Math.abs(Math.cos(R)) + this.drawOps.size : textW, this.drawOps.isRotate
    ...
    drawWaterMark(ops) {
        ...
        if (!this.drawOps.text)
            return;
        const workspace = this.canvas.getObjects().find((item) => item.id === 'workspace');
        const { width, height, left, top } = workspace;
        this.drawing[(_a = this.drawOps) === null || _a === void 0 ? void 0 : _a.position](width, height, (imgString) => {
